...
const CreateInvoice = () => {
  const { createInvoiceAppointment } = useStaff();
  const [appointmentId, setAppointmentId] = useState<number>(0);
  const router = useRouter();
  const createInvoiceHandler = useCallback(async (id: number) => {
    if (id === 0) {
      Toast.show({
    ...
    await createInvoiceAppointment(
      id,
      () => {
        Toast.show({
      ...
      <ScrollView contentContainerStyle={styles.container}>
        <View style={{ marginBottom: 20 }}>
          ...
          <ButtonCustom
            mt={10}
            title="Tạo"
            onPress={async () => createInvoiceHandler(appointmentId)}
          />
        </View>
      </ScrollView>
    </SafeAreaView>
